...
use crate::{
    clear_suid_sgid, creation_gid, time_from_system_time, xattr_access_check, DirectoryDescriptor,
    FileKind, Inode, InodeAttributes, BLOCK_SIZE, FILE_HANDLE_READ_BIT, FILE_HANDLE_WRITE_BIT,
    FMODE_EXEC, MAX_FILE_SIZE, MAX_NAME_LENGTH,
};
...
impl SimpleFS {
    pub fn new(
        data_dir: String, direct_io: bool, #[allow(unused_variables)] suid_support: bool,
    ) -> SimpleFS {
        SimpleFS {
            data_dir,
    ...
    fn truncate(
        &self, inode: Inode, new_length: u64, uid: u32, gid: u32,
    ...
    fn insert_link(
        &self, req: &Request, parent: u64, name: &OsStr, inode: u64, kind: FileKind,
    ) -> Result<(), c_int> {
...
impl Filesystem for SimpleFS {
    fn init(
        &mut self, _req: &Request, #[allow(unused_variables)] config: &mut KernelConfig,
    ) -> Result<(), c_int> {
    ...
    fn setattr(
        &mut self, req: &Request, inode: u64, mode: Option<u32>, uid: Option<u32>,
        gid: Option<u32>, size: Option<u64>, atime: Option<TimeOrNow>, mtime: Option<TimeOrNow>,
        _ctime: Option<SystemTime>, fh: Option<u64>, _crtime: Option<SystemTime>,
    ...
    fn mknod(
        &mut self, req: &Request, parent: u64, name: &OsStr, mut mode: u32, _umask: u32,
    ...
    fn symlink(
        &mut self, req: &Request, parent: u64, name: &OsStr, link: &Path, reply: ReplyEntry,
    ) {
    ...
    fn rename(
        &mut self, req: &Request, parent: u64, name: &OsStr, new_parent: u64, new_name: &OsStr,
        flags: u32, reply: ReplyEmpty,
    ) {
    ...
    fn link(
        &mut self, req: &Request, inode: u64, new_parent: u64, new_name: &OsStr, reply: ReplyEntry,
    ) {
        debug!(
            "link() called for {}, {}, {:?}",
    ...
    fn read(
        &mut self, _req: &Request, inode: u64, fh: u64, offset: i64, size: u32, _flags: i32,
    ...
    fn write(
        &mut self, _req: &Request, inode: u64, fh: u64, offset: i64, data: &[u8],
        _write_flags: u32, #[allow(unused_variables)] flags: i32, _lock_owner: Option<u64>,
        reply: ReplyWrite,
    ) {
    ...
    fn release(
        &mut self, _req: &Request<'_>, inode: u64, _fh: u64, _flags: i32, _lock_owner: Option<u64>,
    ...
    fn readdir(
        &mut self, _req: &Request, inode: u64, _fh: u64, offset: i64, mut reply: ReplyDirectory,
    ) {
    ...
    fn releasedir(
        &mut self, _req: &Request<'_>, inode: u64, _fh: u64, _flags: i32, reply: ReplyEmpty,
    ) {
        if let Ok(mut attrs) = self.get_inode(inode) {
            attrs.open_file_handles -= 1;
        }
    ...
    fn setxattr(
        &mut self, request: &Request<'_>, inode: u64, key: &OsStr, value: &[u8], _flags: i32,
    ...
    fn getxattr(
        &mut self, request: &Request<'_>, inode: u64, key: &OsStr, size: u32, reply: ReplyXattr,
    ) {
    ...
    #[cfg(target_os = "linux")]
    fn fallocate(
        &mut self, _req: &Request<'_>, inode: u64, _fh: u64, offset: i64, length: i64, mode: i32,
        reply: ReplyEmpty,
    ) {
        let path = self.content_path(inode);
    ...
    fn copy_file_range(
        &mut self, _req: &Request<'_>, src_inode: u64, src_fh: u64, src_offset: i64,
        dest_inode: u64, dest_fh: u64, dest_offset: i64, size: u64, _flags: u32, reply: ReplyWrite,
    ) {
        debug!(
...
pub fn check_access(
    file_uid: u32, file_gid: u32, file_mode: u16, uid: u32, gid: u32, mut access_mask: i32,
) -> bool {
